...
manage(HWND hwnd) {    
    ...
    if (c->isfloating && IsWindowVisible(hwnd)) {
        ...
        resize(c, wi.rcWindow.left, wi.rcWindow.top, wi.rcWindow.right - wi.rcWindow.left, wi.rcWindow.bottom - wi.rcWindow.top,0,0,0,0);
    }
...
monocle(void) {
    ...
    for (c = nexttiled(clients); c; c = nexttiled(c->next)) {
        resize(c, wx - 10, wy - 5, (ww - 2 * c->bw) + 16, (wh - 2 * c->bw) + 10, 0,0,0,0);
    }
}
...
void
resize(Client *c, int x, int y, int w, int h, int xoff, int yoff, int woff, int hoff) {
    if (w <= 0 && h <= 0) {
...
tile(void) {
    ...
    c = nexttiled(clients);
    mw = mfact * ww;
    resize(c, wx, wy, (n == 1 ? ww : mw) - 2 * c->bw, wh - 2 * c->bw, 10,10,10,15);
...
togglefloating(const Arg *arg) {
    ...
    if (sel->isfloating)
        resize(sel, sel->x, sel->y, sel->w, sel->h,0,0,0,0);
    arrange();
}
